...
#Plot power spectrum
raw.plot_psd(fmax=180, n_fft=250)
...
prep = prep.filter(1, 80, picks='eeg', filter_length='auto', 
                 l_trans_bandwidth='auto', h_trans_bandwidth='auto', 
                 method='fir', phase='zero', fir_window='hamming', 
                 fir_design='firwin')

#Plot power spectra
prep.plot_psd(fmax=140)
...
### The comparison might be easier to do if we call 
### mne.time_frequency.psd_array_welch() directly on each object and take a 
...
ica.fit(prep, picks='eeg')

#Quick look at components
ica.plot_components(inst=prep, plot_std=False,
                    picks='eeg',
                    psd_args=dict(fmax=110))

#Plot time course of ICs
ica.plot_sources(prep)
...
ica.plot_overlay(prep, start=0.0, stop=4550*256, exclude=idx_bad, picks='eeg')
ica.exclude = idx_bad
ica.apply(prep)
...
ax.set_xlim(0, 140)
ax.set_ylim(-70, 50)
#raw.plot_psd(fmax=140, ax=ax)
prep.plot_psd(fmax=140, ax=ax)
...
cathodes = ['F8', 'T4', 'T6', 'O2', 'F7', 'T3', 'T5', 'O1', 
            ...
            'T2', 'T1']
# =============================================================================
# #Set left and right ear reference
# anodes = ['T1','F7','T3','T5','Fp1','F3','C3','P3','O1',
#           'Fz','Pz',
#           'T2','F8','T4','T6','Fp2','F4','C4','P4','O2']
# cathodes = ['A1','A1','A1','A1','A1','A1','A1','A1','A1',
#             'Cz','Cz',
...
prep_bi = mne.set_bipolar_reference(prep_bi, anodes, cathodes)
